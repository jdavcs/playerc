...
public class Grammar {
  ...
  public ParsingTable getParsingTable() {
    ...
    for (int i = 0; i < productions.size(); i++) {
      ...
      while (iFirsts.hasNext()) {
        ...
        try {
          table.add(symbolLeft, terminal, action);
      ...
      if (firstsSubset.contains(nullSymbol)) {
        ...
        while (iFollows.hasNext()) {
          ...
          // add only if terminal is not in firsts -> because then it's right
          // recursion?
  ...
  // add all symbols from grammar and end-marker to terminals except
  // nonterminals
