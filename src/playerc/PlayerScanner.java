 ...
 * @version 3.1 -- created: 2005-06-22 modified: 2017-09-04
 */
public class PlayerScanner extends Scanner {
	...
	private Token q1(String s) throws IOException, LexicalException {
		...
		if (isNumericDigit(c))
			...
			if (val <= MAX_INT_LENGTH) {
				...
				throw new LexicalException("Identifier longer than max (" + MAX_ID_LENGTH + "): " + val);
		}
	}
	...
	private Token q8(String s) throws IOException, LexicalException {
		...
		if (isLetter(c) || isNumericDigit(c)) {
			counter++;
			return q8(s + c);
		} else if (counter <= MAX_ID_LENGTH) {
			counter = 0;
			unread(c);
			return tokenHelper.getWordToken(s);
		} else
			throw new LexicalException("Identifier longer than max (" + MAX_ID_LENGTH + "): " + s);
	}
