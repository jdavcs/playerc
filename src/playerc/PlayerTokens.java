 ...
 * 
 * DO NOT remove single quotes from keywords - there are keywords which have the same spelling as nonterminals, so the 
 * single quotes distinguish terminals from nonterminals and other elements of the grammar!
 */
public class PlayerTokens
{
   ...
   private HashMap wordTokens; //predefined keywords + user-defined identifiers (for scanner) (Token objects stored by token names)
   ...
   public Token getWordToken(String s) //used by scanner
   {
      ...
      if (t == null) //if hasn't found - create new token and add to table, then return
      {
   ...
   public int getTokenType(String s) { return tokenNames.indexOf(s); }
 
   public String getTokenName(int type) { return (String)tokenNames.elementAt(type); }
   ...
   private void initTokenNames() //loads predefined keywords, symbols and punctuation
   {
