...
public class TypeChecker implements Visitor
{
	...
	public void check(Program p) { p.accept(this); }
		
	public void visit(Program x) 
	{
		Identifier id = x.id();
		
		DeclarationList decls = x.declarations();
		StatementList stms = x.statements();
		
		//load symbol table
		
		
		
		//first run thru the decls and load symbol table with ids+TypeExps
		//themn check the program. each id - lookup in the symbol table and assign typeexp
